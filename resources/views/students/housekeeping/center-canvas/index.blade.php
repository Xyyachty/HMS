<div class="flex-1 min-h-0 p-4 flex flex-col">
    ...
    <div class="flex-1 flex flex-col items-center justify-center gap-8">
        <div class="text-center">
            <p class="text-[10px] font-semibold tracking-widest text-teal-500 uppercase mb-2">Housekeeping Services</p>
    ...
    <div id="canvasFrame" class="flex-1 min-h-0 {{ !$hasTemplate ? 'hidden' : '' }}">
        <div class="canvas-frame h-full">
            ...
            <div class="flex-1 min-h-0 bg-white">
                <iframe id="templateFrame" src="{{ $selectedTemplate ? route('students.frontdesk.template.' . $selectedTemplate, ['role' => $builderRole ?? 'housekeeping']) : '' }}" title="Housekeeping Template" class="w-full h-full border-0"></iframe>
            </div>
        </div>
    </div>
