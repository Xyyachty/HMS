        ...
        <table class="w-full text-sm" style="table-layout:fixed">
            <thead>
                <tr class="bg-slate-50 border-b border-slate-200">
                    ...
                    <th class="text-left px-4 py-2.5 text-[10px] font-extrabold uppercase tracking-wider text-slate-500" style="width:22%">Page Name</th>
                    ...
                    <th class="text-left px-4 py-2.5 text-[10px] font-extrabold uppercase tracking-wider text-slate-500" style="width:14%">Final Grade</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($teamReports as $index => $report)
                    <tr class="hover:bg-slate-50/80">
                        <td class="px-4 py-3">
                            ...
                            <p class="text-[11px] text-slate-400 truncate" title="{{ $report['faculty_name'] }}">{{ $report['faculty_name'] }}</p>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-12 text-center text-sm text-slate-400 font-semibold">No completed team assignments to report.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
...
<script>
    ...
    function openTeamReportModal(index) {
        ...
        document.getElementById('teamReportModalSubtitle').textContent = [
            report.faculty_name ? `Faculty: ${report.faculty_name}` : null,
            report.page_name ? `Pages: ${report.page_name}` : null,
        ].filter(Boolean).join(' · ');
